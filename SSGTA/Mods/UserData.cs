...
namespace SSGTA.Mods
{
    public class UserInfo : MonoBehaviour
    {
        ...
        public List<string> CheckCosmetics(VRRig rig)
        {
            ...
            return specialties.IsNullOrEmpty() ? new List<string>{ "<color=#8B0000>No Special Cosmetics Found</color>" } : specialties;
        }
        ...
        public List<string> CheckMods(VRRig rig)
        {
            ...
            return specialMods.IsNullOrEmpty() ? new List<string> { "<color=#8B0000>No Mods Found</color>" } : specialMods;
        }

        // End of II's Code

        public void SpoofProps()
        {
            const string chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
            System.Random rng = new System.Random();
            ExitGames.Client.Photon.Hashtable props = new ExitGames.Client.Photon.Hashtable();

            foreach (string ok in specialModsList.Keys)
            {
                char[] buffer = new char[5];
                for (int i = 0; i < buffer.Length; i++) buffer[i] = chars[rng.Next(chars.Length)];
                props[ok] = new string(buffer);
            }
